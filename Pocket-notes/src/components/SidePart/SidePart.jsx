...
function SidePart({ setSelcGrpName, setSelcGrpClr}) {
  const [isModalTrue, setIsModalTrue] = useState(false);
  const [notesGroup, setNotesGroup]= useState({name: '', colorIndex: ''});
  const [createdNotesGroups, setCreatedNotesGroups] =useState(()=>{
    const storedGrps = localStorage.getItem('createdNotesGroups');
    return storedGrps ? JSON.parse(storedGrps) : [];
  });
  ...
  const openModal = ()=>setIsModalTrue(true);
  const closeModal =()=> setIsModalTrue(false);
  ...
  setSelcGrpName(selectedGroup.name);
  setSelcGrpClr(colors[selectedGroup.colorIndex]);
  };
  ...
  return (
    <div className="container1">
        ...
        <div className="Btn">
          ...
          Create Notes Group Here
          </button>
        </div>

        {isModalTrue && <div className='popup'></div>}
        <Modal
          isOpen={isModalTrue} onRequestClose={closeModal}  contentLabel="Create New Group" className='modal-content' >
            <div className="model-data">
              <h3 style={{padding: '15px 5px'}}>Create new group</h3>
